package com.bruno.javacore.ZZIjdbc.repository;
...
public class ProducerRepository {
    ...
    public static void delete(int id) {
        String sql = "DELETE FROM `anime_store`.`producer` WHERE (`id` = '2');".formatted(id);

        try (Connection conn = ConnectionFactory.getConnection();
             Statement stmt = conn.createStatement()) {
            int rowsAffected = stmt.executeUpdate(sql);
            log.info("Deleted producer '{}' from in the database, rows affected '{}' ", id, rowsAffected);
        } catch (SQLException e) {
        ...
        try (Connection conn = ConnectionFactory.getConnection()) {
            ...
            if (dbMetaData.supportsResultSetType(ResultSet.TYPE_SCROLL_INSENSITIVE)) {
                ...
                if (dbMetaData.supportsResultSetConcurrency(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE)) {
                    log.info("And supports TYPE_FORWARD_ONLY");
